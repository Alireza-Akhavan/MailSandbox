...
# --- CORS (dev-friendly; adjust if you want to restrict) ---
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# --- Static files (mount if folder exists) ---
# Supports email_server/static or <repo_root>/static
_THIS_DIR = Path(__file__).resolve().parent               # email_server/
_TEMPLATES_DIR = _THIS_DIR / "templates"
_REPO_ROOT = _THIS_DIR.parent                              # repository root (where ui_all.html is)
_STATIC_CANDIDATES = [
    _THIS_DIR / "static",
    _REPO_ROOT / "static",
]
for _static_dir in _STATIC_CANDIDATES:
    if _static_dir.exists() and _static_dir.is_dir():
        ...
        break  # mount the first one that exists

# --- UI: serve ui_all.html at "/" ---
# Search first in root (as in your structure), then in email_server/templates
_UI_CANDIDATES = [
...
async def serve_ui(request: Request):
    load_dotenv()
    ui_email_server = os.getenv("EMAIL_SERVER_API_URL", "http://127.0.0.1:8000")
